...
public class ChartGraphSuhuTanahActivity extends AppCompatActivity {
    private LineChart mChart;
    ArrayList<SenseSuhuTanah> arrSuhuTanah = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chartMakerService.getSuhuTanahChartData(isiSpinner, sinceList).enqueue(new Callback<ArrayList<SenseSuhuTanah>>() {
            @Override
            public void onResponse(Call<ArrayList<SenseSuhuTanah>> call, Response<ArrayList<SenseSuhuTanah>> response) {
                ArrayList<SenseSuhuTanah> arrayList = response.body();
                for (int i = 0; i < Integer.parseInt(cred.loadJumlahNode()); i++) {
                    arrSuhuTanah.add(new SenseSuhuTanah(arrayList.get(i).getKode_petak(), arrayList.get(i).getSuhu_Tanah(), arrayList.get(i).getWaktu_sensing()));
                    ArrayList<Entry> node = new ArrayList<>();
                    String secsStr = arrayList.get(i).getWaktu_sensing();
                    node.add(new Entry(getSecond(secsStr), Float.parseFloat(arrayList.get(i).getSuhu_Tanah())));
...
//                    arrLineDataSet.get(i).setDrawValues(false);
                    ...
                    public void run() {
                        try {
                            if (done) {
                                ...
                                ChartGraphSuhuTanahActivity.ChartGraphAsyncTask chartGraphAsyncTask = new ChartGraphAsyncTask();
    ...
    private class ChartGraphAsyncTask extends AsyncTask<String, Void, ArrayList<SenseSuhuTanah>> {
        @Override
        protected ArrayList<SenseSuhuTanah> doInBackground(String... strings) {
            ChartMakerService chartMakerService = AppAPI.getRetrofit().create(ChartMakerService.class);
            try {
                ArrayList<SenseSuhuTanah> arrTemp = chartMakerService.getSuhuTanahChartData(isiSpinner, sinceList).execute().body();
                for (int i = 0; i < arrTemp.size(); i++) {
                    int kodePetak = Integer.parseInt(arrTemp.get(i).getKode_petak());

                    arrSuhuTanah.get(kodePetak - 1).setKode_petak(arrTemp.get(i).getKode_petak());
                    arrSuhuTanah.get(kodePetak - 1).setSuhu_tanah(arrTemp.get(i).getSuhu_Tanah());
                    arrSuhuTanah.get(kodePetak - 1).setWaktu_sensing(arrTemp.get(i).getWaktu_sensing());

                    if (sdf.parse(arrSuhuTanah.get(kodePetak - 1).getWaktu_sensing()).after(sdf.parse(sinceList.get(String.format("since[%d]", kodePetak))))) {
                        sinceList.put(String.format("since[%d]", kodePetak), arrSuhuTanah.get(kodePetak - 1).getWaktu_sensing());
                    }
                    LineData data = mChart.getLineData();
                    set = (LineDataSet) data.getDataSetByIndex(kodePetak - 1);
                    set.addEntry(new Entry(getSecond(arrSuhuTanah.get(kodePetak - 1).getWaktu_sensing()), Float.parseFloat(arrSuhuTanah.get(kodePetak - 1).getSuhu_Tanah())));
        ...
        @Override
        protected void onPostExecute(ArrayList<SenseSuhuTanah> result) {
            mChart.invalidate();
            done = true;
        }
    }
